...
Viewer archives use `ceres-viewer-<version>-windows-x64.zip`, `ceres-viewer-<version>-linux-x64.tar.gz` and `ceres-viewer-<version>-linux-arm64.tar.gz`. Extract the package and run `ceres-viewer.exe` on Windows or `./ceres-viewer` on Linux. Linux x64 targets Ubuntu 22.04 or newer and Linux ARM64 targets Ubuntu 24.04 or newer on NVIDIA GB10.
