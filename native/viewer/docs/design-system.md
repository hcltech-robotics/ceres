...
The instrument strip is anchored to the top edge above the scene and is 66 pixels high at standard display scale. It ends where the right sidebar begins, at 80% of the window width. The sidebar reaches the top edge, with its Ceres/Bridge title and tracking lamps in a header of the same height. Seven exclusive sections follow: Connection, Hands, Depth, Task, Recording, Telemetry and Calibration. Opening a section closes the previous one, and the active section can also be closed. The selection persists between runs. Section numbers provide orientation. Tab hides the sidebar and expands the scene and instrument strip to the full width.

All seven headers remain visible and span the pane edge to edge. Expanded content uses its natural height until it reaches the available space, then scrolls with the wheel, touchpad or keyboard without a visible scrollbar. Tracking signals and recording controls remain outside the scrolling content. Section contents retain twelve-pixel horizontal padding. Secondary disclosures have a quieter treatment.
...
The Depth layer accumulates a persistent map in the tracking world. Adaptive detail groups distant cells, and memory pressure coarsens stored geometry before pruning lower-value cells. Quest depth uses a warm-near, cool-far spectral scale. Opacity reflects confidence and decreases only when new observations contradict a surface. Detail, Voxel, Mask hands, Clear map and Opacity precede the depth range and its colour scale. Stereo adds Update and Pair limit. Video and rendering retain their own cadence.
...
Body text uses a clear sans serif face. Aligned values, units and secondary markings use a monospaced face when available. Labels use sentence case and physical units remain next to their values. Text is concise, with additional explanation in tooltips.
...
The pairing code sits beside the QR image when both fit. Scene layers use aligned columns which become a single column in narrow panes. Slider labels share a measured gutter and stack above their fields when necessary. Related controls use tight spacing, while rules separate task groups.
