...
The pairing QR image uses the full pane width, with the access code below it. The image and **Enlarge QR code** action open a view sized to the whole window. Both views draw whole framebuffer pixels per module and retain a four-module white quiet zone. Escape, **Close** or the window close control dismisses the enlarged view. Inspector selectors and sliders share a 96-pixel label gutter and a full-width control column. The entire field stacks when fewer than 152 pixels remain for its control or when its label outgrows the gutter. Label and control baselines align in the two-column layout. Stacked labels wrap at the available width, while long text and path inputs retain the full pane width below a persistent label.
