...
void check_scene_navigation(ceres::Renderer& renderer) {
    ...
    for (auto view : {View::top, View::side, View::iso}) {
        require(renderer.select_scene_view(view), "Available view preset was rejected");
        const auto camera = renderer.scene_camera();
        const auto direction = glm::normalize(camera.target - camera.eye);
        require(std::isfinite(glm::length(direction)) &&
                    glm::length(glm::cross(direction, camera.up)) > .1f,
                "Camera preset has a singular view basis");
        if (view == View::iso)
            require(std::abs(std::abs(direction.x) - std::abs(direction.y)) < .0001f &&
                        std::abs(std::abs(direction.y) - std::abs(direction.z)) < .0001f,
