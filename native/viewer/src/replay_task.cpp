...
ReplayTaskTimeline ReplayTaskTimeline::from_events(const std::vector<SessionEvent>& events,
    ...
    boundaries.reserve(result.tasks_.size() * 2);
    for (std::size_t i = 0; i < result.tasks_.size(); ++i) {
        boundaries.push_back({result.tasks_[i].start_us, i, true});
        boundaries.push_back({result.tasks_[i].end_us, i, false});
    }
    std::sort(boundaries.begin(), boundaries.end(), [](const Boundary& first, const Boundary& second) {
        return first.time_us < second.time_us;
    });
    std::set<std::size_t> active;
    std::int64_t previous_us = 0;
    std::size_t next = 0;
    while (next < boundaries.size()) {
        const auto time_us = boundaries[next].time_us;
        if (!active.empty() && previous_us < time_us) {
            const auto task_index = *active.rbegin();
            auto& spans = result.visible_spans_;
            if (!spans.empty() && spans.back().end_us == previous_us &&
                spans.back().task_index == task_index)
                spans.back().end_us = time_us;
            else
                spans.push_back({previous_us, time_us, task_index});
        }
        do {
            const auto& boundary = boundaries[next++];
            if (boundary.start)
                active.insert(boundary.task_index);
            else
                active.erase(boundary.task_index);
        } while (next < boundaries.size() && boundaries[next].time_us == time_us);
        previous_us = time_us;
    }
    return result;
}
...
const ReplayTask* ReplayTaskTimeline::at(std::int64_t position_us) const {
    auto after = std::upper_bound(visible_spans_.begin(), visible_spans_.end(), position_us,
                                  [](std::int64_t position, const ReplayTaskSpan& span) {
                                      return position < span.start_us;
                                  });
    if (after == visible_spans_.begin())
        return nullptr;
    const auto& span = *--after;
    return position_us < span.end_us ? &tasks_[span.task_index] : nullptr;
}
...
const std::vector<ReplayTaskSpan>& ReplayTaskTimeline::visible_spans() const {
    return visible_spans_;
}
