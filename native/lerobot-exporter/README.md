...
`source_dimensions: true` selects the encoded camera resolution automatically. The helper probes the H264 configurations used by the selected ranges and validates every selected decoded image. Capture metadata remains in provenance because WebRTC can encode at a smaller resolution than the camera track. Width and height supply the black-frame dimensions when the selected ranges contain no camera images. The `source_video_dimensions` capability identifies helpers that support this option.

Without `source_dimensions`, width and height must match the encoded source throughout the selected ranges. Export preserves source dimensions. When a recording changes resolution, select ranges at one resolution for each export. Other resolutions outside the selected ranges do not prevent export.
