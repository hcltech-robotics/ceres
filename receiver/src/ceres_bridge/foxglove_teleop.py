...
DIAGNOSTIC_SCHEMA = {"$schema": "http://json-schema.org/draft-07/schema#", "title": "ceres.RobotDiagnostics",
    **object_schema({"model": STRING, "backend": STRING, "update_fps": NUMBER, "solve_ms": NUMBER,
...
async def run_robot(args, stop, *, context=None):
    ...
    try:
        receiver = await asyncio.to_thread(Receiver, args.socket, video=False)
        ...
        while not stop.is_set():
            ...
            result, links = await asyncio.to_thread(step)
            now = time.time_ns()
            ...
            if now - last_diagnostic >= 200_000_000:
                values = {"model": result["model"], "backend": result["backend"], "update_fps": rate,
